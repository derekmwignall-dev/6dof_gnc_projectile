...
public:
	Forces(double mass, double cross_sec, double drag_coefficient, double charge, Matrix3x3 inertia_tensor)
		: m_mass{ mass }, m_cross_sec{ cross_sec }, m_drag_coefficient{ drag_coefficient }, m_charge{ charge }, m_inertia_tensor{ inertia_tensor } {
	}
	...
	{
		return Vector3{ 0.0, 0.0, -Constants::gravity } * m_mass; // only acts in -z-direction (downwards)
	}

	// Function for drag forces
	Vector3 dragForce(const Vector3& velocity) const
	{
		double coefficient { 0.5 * Constants::airDensity * m_cross_sec * m_drag_coefficient }; // calculate scalar value of drag force
	...
	{
		return m_inertia_tensor.inv() * torque; // calculates angular acceleration based on the inverse of the inertia tensor and the torque, which can be used to update the angular velocity
	}
};
...
namespace Inertia {
	...
	inline Matrix3x3 cube(double mass, double side) {
