...
void rk4Step(ProjectileMotion& projectile, const Forces& forces,
	const Vector3& E, const Vector3& B, double dt, const Vector3& r)
{
	// Compute k1 values

	Vector3 k1_vel{ projectile.getVelocity() };
	Vector3 f1{ forces.gravForce() + forces.dragForce(k1_vel) + forces.lorentzForce(k1_vel, E, B) };
	...
	Vector3 f2{ forces.gravForce() + forces.dragForce(k2_vel) + forces.lorentzForce(k2_vel, E, B) };
	...
	Vector3 f3{ forces.gravForce() + forces.dragForce(k3_vel) + forces.lorentzForce(k3_vel, E, B) };
	...
	Vector3 f4{ forces.gravForce() + forces.dragForce(k4_vel) + forces.lorentzForce(k4_vel, E, B) };
	...
	Quaternion delta_orient{ Quaternion::fromAxisAngle(delta_ang_vel.normalize(), delta_ang_vel.magnitude() * dt) };
